...
# Lightweight presence detector — used ONLY when unlocked (~2ms vs ~30ms MediaPipe)
# Only needs to answer "is someone still there?" — no landmarks needed.
_haar = cv2.CascadeClassifier(
    cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')

def _haar_has_face(frame):
    """
    Returns (found: bool, box_or_None).
    Intentionally loose (minNeighbors=2) so a brief head-turn doesn't relock.
    ~2ms on Jetson vs ~30ms for full MediaPipe landmarker.
    """
...
class FaceAuth:
    ...
    # =================================================================
    # NEW: Lightweight presence check - runs every frame when unlocked
    # =================================================================
    def check_presence(self, frame, box):
        """
        Lightweight face presence check (no recognition, ~2-5ms).
        Call this EVERY frame when unlocked to maintain tracker.
        Returns annotated frame (same as input, just drawn box).
        """
        if box is not None:
            # Update tracker with face position
            track_id, is_new = self._tracker.update(box)
            self._grace_count = 0
            
            # Draw green box for unlocked state
            x1, y1, x2, y2 = box
    ...
    def process_frame(self, frame, key):
        ...
        # ── UNLOCKED: skip MediaPipe entirely, use ~2ms haar presence check ──
        # Only pays full MediaPipe cost again after face is confirmed gone.
        if self._unlocked and self._state == _ST_RECOGNISE:
            found, box = _haar_has_face(frame)
            return self.check_presence(frame, box if found else None)
    ...
    def _state_enrolling(self, frame, result, lm, key):
        ...
        cv2.putText(frame, f"{int(prog*100)}% — 30-50cm, look straight | {remaining:.0f}s left",
    ...
    def draw_status_bar(self, frame):
        ...
        if self._unlocked:
            ...
            cv2.putText(frame, f"Session: {int(rem)}s", (frame.shape[1]-160,42),
    ...
    def draw_debug(self, frame):
        ...
        lines = [
            ...
            (f"{self._track_status[:15]}", (180, 180, 180)),
        ]
