...
Optimized for Jetson Orin Nano - Frame skipping added
"""
...
class GestureControl:
    ...
    def process_frame(self, frame, mqtt, face_unlocked):
        ...
        # Only process gesture when unlocked, and skip frames
        if face_unlocked:
            if self._frame_counter % config.GESTURE_PROCESS_EVERY_N_FRAMES != 0:
                # Still draw devices on skipped frames
    ...
    def _do_detection(self, frame, detected):
        ...
        if actionable:
            ...
            cv2.putText(frame, f"Hold {remaining:.1f}s...",
                        (20, 196), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255,255,100), 2)
            ...
            cv2.putText(frame, f"-> {dev.upper()} {act.upper()}",
                        (20, 244), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (180,255,180), 2)
    ...
    def _do_confirm(self, frame, detected, mqtt):
        ...
        cv2.putText(frame, "CONFIRM ACTION?",
                    (20,155), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,255), 2)
        cv2.putText(frame, f"{self._cur_gesture} -> {dev} {act}",
                    (20,192), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (255,255,0), 2)
        cv2.putText(frame, "Thumb UP = YES   Thumb DOWN = NO",
                    (20,226), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (200,200,200), 1)
        ...
        if since < config.CONFIRM_ENTRY_DELAY:
            ...
            cv2.putText(frame, f"Stabilising... {rem:.1f}s",
                        (20,260), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (160,160,160), 1)
            self._conf_gesture = None
            self._conf_start   = 0.0
            return None
        ...
        if detected in ("Thumb Up", "Thumb Down"):
            ...
            cv2.putText(frame, f"{lbl} hold {rem:.1f}s",
                        (20,260), cv2.FONT_HERSHEY_SIMPLEX, 0.8, bc, 2)
