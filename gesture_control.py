...
Handles:
  ...
  - Hold-to-confirm system before sending ANY command
...
def detect_gesture(lm):
    ...
    if ie and not me and not re and not pe:
        ...
        if (il > _dist(middle_tip, middle_mcp) + 0.03 and 
            il > _dist(ring_tip, ring_mcp) + 0.03):
...
class GestureControl:
    ...
    def process_frame(self, frame, mqtt, face_unlocked, skip_inference=False):
        """Main processing function - called every frame from main.py"""
        # 1. AI Inference
        detected = "IDLE"
        if not skip_inference:
            ...
            # When skipping inference, keep using last known gesture
            detected = self._smooth()
    ...
    def _do_detection(self, frame, detected):
        if detected in config.GESTURE_COMMANDS:
            ...
            cv2.rectangle(frame, (20, 210),
                          (20 + int(bw * min(elapsed / config.GESTURE_HOLD_TIME, 1.0)), 228),
                          (0, 200, 255), -1)
    ...
    def _do_confirm(self, frame, detected, mqtt):
        ...
        cv2.putText(frame, "CONFIRM ACTION?", (20, 155),
