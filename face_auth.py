...
def face_thread(face, buf, state, stop_event):
    """
    Non-blocking design — the display NEVER freezes:

      1. Grab latest raw frame.
      2. Immediately write it to face_buf so gesture/display threads
         always have something fresh — no waiting on recognition.
      3. Then run the slow recognition on a local copy.
      4. After recognition finishes, overwrite face_buf with the
         annotated result.

    Worst case: annotation is 1 recognition-cycle old, NOT a freeze.

    Scheduling:
      LOCKED   → run MediaPipe every frame (identify new face ASAP)
      UNLOCKED → run MediaPipe every 90 frames (just checking presence)
                 In between, raw frame passes straight through.
    """
    frame_n = 0
    while not stop_event.is_set():
        ...
        # STEP 1: Push raw immediately so display/gesture never stall
        buf.write_face(raw)
        ...
        # STEP 2: Decide whether to run heavy recognition this frame
        run_recognition = (
            not unlocked          # locked  → run every frame
            or frame_n % 90 == 0  # unlocked → periodic presence check
            or key != -1          # key pressed → always process
        )
        ...
        # STEP 3: Run recognition on a private copy (may take 20-40ms)
        work      = raw.copy()
        annotated = face.process_frame(work, key)
        face.handle_key(key)
        state.set_auth(face.is_unlocked(), face.unlocked_name())

        # STEP 4: Push annotated result (replaces the raw written in step 1)
